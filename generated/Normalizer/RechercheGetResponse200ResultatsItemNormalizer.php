...
class RechercheGetResponse200ResultatsItemNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('dirigeants') && null !== $data->getDirigeants()) {
            $values_2 = [];
            foreach ($data->getDirigeants() as $value_2) {
                $values_2[] = $this->normalizer->normalize($value_2, 'json', $context);
            }
            $dataArray['dirigeants'] = $values_2;
        }
        if ($data->isInitialized('beneficiaires') && null !== $data->getBeneficiaires()) {
            $values_3 = [];
            foreach ($data->getBeneficiaires() as $value_3) {
                $values_3[] = $this->normalizer->normalize($value_3, 'json', $context);
            }
            $dataArray['beneficiaires'] = $values_3;
        }
        if ($data->isInitialized('documents') && null !== $data->getDocuments()) {
            $values_4 = [];
            foreach ($data->getDocuments() as $value_4) {
                $values_4[] = $this->normalizer->normalize($value_4, 'json', $context);
            }
            $dataArray['documents'] = $values_4;
        }
        if ($data->isInitialized('publications') && null !== $data->getPublications()) {
            $values_5 = [];
            foreach ($data->getPublications() as $value_5) {
                $values_5[] = $this->normalizer->normalize($value_5, 'json', $context);
            }
            $dataArray['publications'] = $values_5;
        }
        if ($data->isInitialized('nbDirigeantsTotal') && null !== $data->getNbDirigeantsTotal()) {
            $dataArray['nb_dirigeants_total'] = $data->getNbDirigeantsTotal();
        }
        if ($data->isInitialized('nbBeneficiairesTotal') && null !== $data->getNbBeneficiairesTotal()) {
            $dataArray['nb_beneficiaires_total'] = $data->getNbBeneficiairesTotal();
        }
        if ($data->isInitialized('nbDocumentsAvecMentions') && null !== $data->getNbDocumentsAvecMentions()) {
            $dataArray['nb_documents_avec_mentions'] = $data->getNbDocumentsAvecMentions();
        }
        if ($data->isInitialized('nbDocumentsTotal') && null !== $data->getNbDocumentsTotal()) {
            $dataArray['nb_documents_total'] = $data->getNbDocumentsTotal();
        }
        if ($data->isInitialized('nbPublicationsAvecMentions') && null !== $data->getNbPublicationsAvecMentions()) {
            $dataArray['nb_publications_avec_mentions'] = $data->getNbPublicationsAvecMentions();
        }
        if ($data->isInitialized('nbPublicationsTotal') && null !== $data->getNbPublicationsTotal()) {
            $dataArray['nb_publications_total'] = $data->getNbPublicationsTotal();
        }
        foreach ($data as $key => $value_6) {
            if (preg_match('/.*/', (string) $key)) {
