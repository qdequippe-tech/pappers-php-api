...
class AppelOffreLanceNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use CheckArray;
    ...
    {
        return AppelOffreLance::class === $type;
    }

    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return \is_object($data) && AppelOffreLance::class === $data::class;
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('titulaires', $data) && null !== $data['titulaires']) {
            $values = [];
            foreach ($data['titulaires'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, AppelOffreEntreprise::class, 'json', $context);
            }
            $object->setTitulaires($values);
            unset($data['titulaires']);
        } elseif (\array_key_exists('titulaires', $data) && null === $data['titulaires']) {
            $object->setTitulaires(null);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('titulaires') && null !== $data->getTitulaires()) {
            $values = [];
            foreach ($data->getTitulaires() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $dataArray['titulaires'] = $values;
        }
        foreach ($data as $key => $value_1) {
            if (preg_match('/.*/', (string) $key)) {
    ...
    {
        return [AppelOffreLance::class => false];
    }
}
