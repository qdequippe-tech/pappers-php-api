...
class RepresentantEntrepriseNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return \is_object($data) && RepresentantEntreprise::class === $data::class;
    }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('decede', $data) && \is_int($data['decede'])) {
            $data['decede'] = (bool) $data['decede'];
        }
        ...
        if (\array_key_exists('sanctions', $data) && null !== $data['sanctions']) {
            $values_2 = [];
            foreach ($data['sanctions'] as $value_2) {
                $values_2[] = $this->denormalizer->denormalize($value_2, Sanction::class, 'json', $context);
            }
            $object->setSanctions($values_2);
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('sanctions') && null !== $data->getSanctions()) {
            $values_2 = [];
            foreach ($data->getSanctions() as $value_2) {
                $values_2[] = $this->normalizer->normalize($value_2, 'json', $context);
            }
            $dataArray['sanctions'] = $values_2;
        }
        if ($data->isInitialized('decede') && null !== $data->getDecede()) {
            $dataArray['decede'] = $data->getDecede();
        }
        if ($data->isInitialized('dateDeDeces') && null !== $data->getDateDeDeces()) {
            $dataArray['date_de_deces'] = $data->getDateDeDeces();
        }
        foreach ($data as $key => $value_3) {
            if (preg_match('/.*/', (string) $key)) {
