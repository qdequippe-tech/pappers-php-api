...
class EntrepriseFicheFinancesItemNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('taux_croissance_effectif', $data) && \is_int($data['taux_croissance_effectif'])) {
            $data['taux_croissance_effectif'] = (float) $data['taux_croissance_effectif'];
        }
        ...
        if (\array_key_exists('date_de_cloture_exercice', $data) && null !== $data['date_de_cloture_exercice']) {
            $date = \DateTime::createFromFormat('Y-m-d', $data['date_de_cloture_exercice']);
            if (false === $date) {
                throw new InvalidDateException($data['date_de_cloture_exercice'], 'Y-m-d');
            }
            $object->setDateDeClotureExercice($date->setTime(0, 0, 0));
        ...
        if (\array_key_exists('taux_croissance_effectif', $data) && null !== $data['taux_croissance_effectif']) {
            $object->setTauxCroissanceEffectif($data['taux_croissance_effectif']);
            unset($data['taux_croissance_effectif']);
        } elseif (\array_key_exists('taux_croissance_effectif', $data) && null === $data['taux_croissance_effectif']) {
            $object->setTauxCroissanceEffectif(null);
            unset($data['taux_croissance_effectif']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('tauxCroissanceEffectif') && null !== $data->getTauxCroissanceEffectif()) {
            $dataArray['taux_croissance_effectif'] = $data->getTauxCroissanceEffectif();
        }
